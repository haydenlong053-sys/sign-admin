...
public class SysMenuServiceImpl implements ISysMenuService
{
    ...
    public List<SysMenu> selectMenusByUser(SysUser user)
    {
        ...
        user.getRoles().forEach(role -> {roleIds.add(role.getRoleId());});
        // 管理员显示所有菜单信息
        if (user.isAdmin() || (!env.equals("prod") && CollectionUtils.isNotEmpty(roleIds) && roleIds.contains(102L)))
        {
    ...
    public List<SysMenu> selectMenuList(SysMenu menu, SysUser sysUser) {
        List<SysMenu> menuList = null;
        List<Long> roleIds = new ArrayList<>();
        sysUser.getRoles().forEach(role -> {roleIds.add(role.getRoleId());});
        if (sysUser.isAdmin() || (!env.equals("prod") && CollectionUtils.isNotEmpty(roleIds) && roleIds.contains(102L)))
        {
            menuList = menuMapper.selectMenuList(menu);
        }
        else
