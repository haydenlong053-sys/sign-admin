...
(function ($) {
    $.extend({
        ...
        table: {
            ...
            // 导出数据
            exportExcelList: function(formId) {
                table.set();
                $.modal.loading("正在导出数据，请稍候...");
            ...
            importTemplate: function() {
                $.get(activeWindow().table.options.downloadTemplate, function(result) {
                    if (result.code == web_status.SUCCESS) {
                        window.location.href = ctx + "common/download?fileName=" + encodeURI(result.msg) + "&delete=" + true;
                    } else if (result.code == web_status.WARNING) {
                        $.modal.alertWarning(result.msg)
            ...
            importExcelKay: function(formId, width, height) {
                table.set();
                var currentId = $.common.isEmpty(formId) ? 'importTpl' : formId;
                var _width = $.common.isEmpty(width) ? "400" : width;
                var _height = $.common.isEmpty(height) ? "230" : height;
        ...
        modal: {
            ...
            reviewTasks: function(type) {
                ...
                $.modal.confirm("确认要审核选中的" + rows.length + "条数据吗?", function() {
                    var url = table.options.batchModerationUrl;
                    var data = { "ids": rows.join(),"type":type};
        ...
        operate: {
            ...
            submitDeposit: function(url, type, dataType, data, callback) {
                var config = {
                    url: url,
                    type: type,
                    dataType: dataType,
            ...
            postDepostit: function(url, data, callback) {
                $.operate.submitDeposit(url, "post", "json", data, callback);
            },
            ...
            recoverPassword: function(id) {
                table.set();
                $.modal.confirm("确定恢复该数据支付密码为(123456)吗？", function() {
                    var url = $.common.isEmpty(id) ? table.options.removeUrl : table.options.removeUrl.replace("{id}", id);
                    if (table.options.type == table_type.bootstrapTreeTable) {
                        $.operate.get(url);
                    } else {
            ...
            onChain: function(id) {
                ...
                if ($.common.isEmpty(id) && table.options.type == table_type.bootstrapTreeTable) {
                    ...
                    var url = table.options.updateUrl.replace("{id}", row[table.options.uniqueId]);
                    $.modal.open("链上订单信息", url);
                } else {
                    $.modal.open("链上订单信息", $.operate.editUrl(id));
                }
            },
            //子订单
            childOrders: function(id) {
                table.set();
                $.modal.open("子订单列表", table.options.logUrl+"?id="+id);
            },
            //我的接单员
            orderTaker: function(id) {
                table.set();
                $.modal.open("接单员", table.options.orderTaker+"?id="+id);
            },
            mySubordinates: function(id) {
                table.set();
                $.modal.open("我的下级", table.options.mySubordinates+"?id="+id);
            },
            releaseDetails: function(id) {
                table.set();
                $.modal.open("释放明细", table.options.prefixLogUrl+"?id="+id);
            },
            subordinateUser: function(id) {
                table.set();
                $.modal.open("下级用户", table.options.subordinateUser+"?id="+id);
            },
            queryForce: function(id) {
                table.set();
                $.modal.open("查看战力", table.options.queryForceUrl+"?id="+id);
            },
            // 修改信息，以tab页展现
            editTab: function(id) {
                table.set();
